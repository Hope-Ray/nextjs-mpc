...
const Modal = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
                ...
                <div className="hero-content flex justify-center items-center">
                  <div className="bg-[#F1F1F6] rounded-lg w-full">
                    <div className="text-xl text-center md:text-3xl font-medium text-brandColor py-2">
                      <span className="whitespace-nowrap flex items-center justify-around">
                        BOOK AN APPOINTMENT
                        <RxCross1
                          role="button"
                          onClick={() => setIsOpen(false)}
      ...
      <div className="fixed top-2 right-[10%] md:right-6 z-50 pointer-events-none flex flex-col gap-1 min-w-72">
        <AnimatePresence>
          {notifications.map((n) => (
            <CustomNotification 
              key={n.id}
